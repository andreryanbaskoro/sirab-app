...
class PermintaanController extends Controller
{
    ...
    {
        $query = Permintaan::forKonsultan(Auth::id())
        ...
        return view('konsultan.permintaan.index', compact('data', 'statuses'));
    }

    public function show(Permintaan $permintaan)
    {
        $this->authorize('view-konsultan', $permintaan);
        $permintaan->load(['konsumen.profile', 'tipeRumah', 'rab.details', 'kontrak', 'validasis']);
        return view('konsultan.permintaan.show', compact('permintaan'));
    }
    ...
    public function terima(Permintaan $permintaan)
    {
        ...
        $permintaan->update(['status' => PermintaanStatus::DITERIMA_KONSULTAN]);
    ...
    public function tolak(Request $request, Permintaan $permintaan)
    {
        ...
        $permintaan->update([
            'status' => PermintaanStatus::DITOLAK_KONSULTAN,
            'alasan_tolak' => $request->alasan_tolak,
        ]);

        try {
            $this->notifService->notifikasiPermintaanDitolakKonsultan($permintaan);
        } catch (\Exception $e) {
            // log error
        }
    ...
    {
        if ($permintaan->konsultan_id !== Auth::id()) {
