...
class AnggaranController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $hargaJasa      = HargaJasaKonsultan::where('user_id', Auth::id())->latest()->get();
        ...
        return view('konsultan.anggaran.index', compact(
    ...
    public function storePekerjaan(Request $request)
    {
        ...
        return redirect()->route('konsultan.anggaran.index', ['tab' => 'pekerjaan'])
    ...
    // ─── HARGA JASA KONSULTAN (milik sendiri) ───────────────────────────────────
    ...
    public function storeJasa(Request $request)
    {
        ...
        return redirect()->route('konsultan.anggaran.index', ['tab' => 'jasa'])
            ->with('success', 'Harga jasa berhasil ditambahkan.');
    }

    public function updateJasa(Request $request, HargaJasaKonsultan $hargaJasaKonsultan)
    {
        // Pastikan hanya pemilik yang bisa edit
        if ($hargaJasaKonsultan->user_id !== Auth::id()) {
            abort(403);
        }
        ...
        $hargaJasaKonsultan->update($request->only('nama_jasa', 'harga', 'deskripsi'));

        return redirect()->route('konsultan.anggaran.index', ['tab' => 'jasa'])
            ->with('success', 'Harga jasa berhasil diperbarui.');
    }

    public function destroyJasa(HargaJasaKonsultan $hargaJasaKonsultan)
    {
        if ($hargaJasaKonsultan->user_id !== Auth::id()) {
            abort(403);
        }
