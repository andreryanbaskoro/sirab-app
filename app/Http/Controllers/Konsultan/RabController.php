...
class RabController extends Controller
{
    ...
    {
        $data = Rab::where('konsultan_id', Auth::id())
            ->with(['permintaan.konsumen', 'permintaan.tipeRumah'])
            ->latest()
            ->paginate(10);

        return view('konsultan.rab.index', compact('data'));
    }

    public function create(Permintaan $permintaan)
    {
        if ($permintaan->konsultan_id !== Auth::id()) abort(403);
        if (!in_array($permintaan->status, [PermintaanStatus::DITERIMA_KONSULTAN, PermintaanStatus::DISUSUN_RAB, PermintaanStatus::DITOLAK_KONSUMEN])) {
        ...
        $jasaKonsultans = HargaJasaKonsultan::where('user_id', Auth::id())->latest()->get();
        $existingRab = $permintaan->rab;

        return view('konsultan.rab.create', compact('permintaan', 'materials', 'pekerjaans', 'jasaKonsultans', 'existingRab'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Update RAB notes
        if ($request->catatan_konsultan) {
            $rab->update(['catatan_konsultan' => $request->catatan_konsultan]);
        }

        $permintaan->update(['status' => PermintaanStatus::DISUSUN_RAB]);

        return redirect()->route('konsultan.rab.show', $rab)->with('success', 'RAB berhasil disimpan.');
    }

    public function show(Rab $rab)
    {
        if ($rab->konsultan_id !== Auth::id()) abort(403);
        $rab->load(['permintaan.konsumen.profile', 'permintaan.tipeRumah', 'details', 'permintaan.kontrak']);
        return view('konsultan.rab.show', compact('rab'));
    }
    ...
    public function submit(Request $request, Rab $rab)
    {
        ...
        // Validate if Konsultan hasn't uploaded the required sketch
        if ($rab->permintaan->sumber_denah === 'dibuatkan_konsultan' && empty($rab->permintaan->dokumen_path)) {
    ...
    {
        if ($rab->permintaan->konsultan_id !== Auth::id()) abort(403);
