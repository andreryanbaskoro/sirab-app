...
<div class="ibox">
    ...
    <div class="ibox-body">
        ...
        <form action="{{ route('konsultan.rab.store') }}" method="POST" id="rabForm" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="font-weight-bold">Biaya Jasa Konsultan (Rp)</label>
                        <select class="form-control mb-2" id="jasaSelect" onchange="setJasaValue()">
                            <option value="0">-- Input Manual --</option>
                            @foreach($jasaKonsultans as $jasa)
                                <option value="{{ $jasa->harga }}">{{ $jasa->nama_jasa }}: Rp {{ number_format($jasa->harga, 0, ',', '.') }}</option>
                            @endforeach
                        </select>
                        <input type="number" class="form-control" name="biaya_jasa_konsultan" id="biaya_jasa_konsultan" value="{{ (float)($existingRab->biaya_jasa_konsultan ?? 0) }}" min="0" onkeyup="calculateGrandTotal()">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="font-weight-bold">Margin Keuntungan Konsultan (%)</label>
            ...
            <div class="form-group mt-3">
                ...
                <textarea name="catatan_konsultan" class="form-control" rows="3" placeholder="Contoh: Perkiraan waktu pengerjaan 3 bulan. Harga material bisa berubah mengikuti harga pasar saat eksekusi.">{{ $existingRab->catatan_konsultan ?? '' }}</textarea>
            </div>
                    ...
                    <div class="row text-right">
                        <div class="col-md-8 offset-md-4">
                            <table class="table table-sm table-borderless">
                                ...
                                <tr>
                                    <td>Profit Konsultan (<span id="profitLabel">0</span>%):</td>
            ...
            <div class="mt-4 text-right">
                <a href="{{ route('konsultan.permintaan.show', $permintaan->id) }}" class="btn btn-secondary mr-2">Batal</a>
...
<script>
    ...
    function calculateGrandTotal() {
        ...
        // Jasa & Tambahan
        const jasa = parseFloat($('#biaya_jasa_konsultan').val()) || 0;
